...
function Saved() {
  const [{ cart, category }] = useStateValue();
  return (
    <main>
      {cart.length <= 0 ? (
        <h1 className="text-4xl text-white p-5 flex justify-center rounded-lg shadow-lg bg-orange-300 uppercase">There is no such as Saved cart</h1>
      ) : (
        <section className="container mt-7 ">
          ...
          <Cart>
            {cart.map(({ name, cost, img, favourite, cart }, cartIndex) => {
              return (
                <div
